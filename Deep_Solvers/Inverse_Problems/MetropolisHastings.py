...
class MetropolisHastingsSampler:
    ...
    def __init__(self, x, y, surrogate=None, nparam=2, sig=1.0, dt_init=0.5, 
                 mean=True, numerical=False, roots=None, vert=30, lam=1e-3, device='cpu'):
        """
        ...
        self.lam = torch.tensor(lam,dtype=torch.float32, device=self.device)  # Regularization parameter
        ...
        self.scaling_term = 1 / (2 + torch.sqrt(2 * torch.pi * self.lam)).to(self.device)  # Calculate once
    ...
    def log_prior_alpha(self, theta):
        ...
        regularization_term = -(torch.clamp(torch.abs(theta) - 1, min=0) ** 2) / (2 * self.lam)
