...
class Burgers(deepGalerkin):
    ...
    @deepGalerkin.laplace_approx()
    def burgers_pde(self, x_interior):
    ...
    def pde_loss(self,data_interior,loss_fn):
        elliptic_pred = self.burgers_pde(data_interior)
    ...
    def ic_loss(self,ic_pts,loss_fn):
        u_ic = self.u(ic_pts)
        left_vals = -torch.sin(torch.pi*ic_pts[:,0].reshape(-1,1))
    ...
    def losses(self,data_interior,ic_pts,bcl_pts,bcr_pts,loss_fn):
        ...
        losses = {"burgers":loss_pde,"uic":loss_uic ,"ubcl":loss_ubcl, "ubcr":loss_ubcr}
        return losses
