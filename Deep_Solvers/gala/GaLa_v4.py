...
class llaplace:
    ...
    def posterior_covariance(self):
        ...
        if self.hessian_structure == "diag":
            ...
            #return torch.linalg.pinv(self.posterior_precision, hermitian=True)
            # cov = torch.linalg.solve(self.posterior_precision, torch.eye(self.n_params))
            # return (cov + cov.T)/2
    ...
    def jacobians_GN (self,pde):
        for cond in pde["PDE"]:
            if cond != "de":
                ...
                self.hessian_pde(pde)

    
    def hessian_pde(self,pde):
        self.model.zero_grad()
    ...
    def diagonal_Hessian(self,pde):
        ...
        cond_ = [i for i in self.loss_laplacian.keys()][0]
        out_dim = self.loss_laplacian[cond_].shape[1]
