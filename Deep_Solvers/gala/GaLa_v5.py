...
class llaplace:
    ...
    def jacobians_GN (self,pde):
        for cond in pde["PDE"]:
            ...
            phi= torch.cat((features,torch.ones(fout.shape[0],1,device=self._device)),1) 

            bsize,output_size = phi.shape[0],  fout.shape[-1]

            identity = torch.eye(output_size, device=self._device).unsqueeze(0).tile(bsize, 1, 1)
    ...
    def full_Hessian(self,pde):
        ...
        for cond in pde["PDE"]:
            ...
            self.H += torch.sum(torch.einsum('bce,bdf->bef', self.jacobians_gn[cond], self.jacobians_gn[cond]),axis=0)
        
        self.H += torch.eye(self.H.shape[0],device=self._device)*1e-4
