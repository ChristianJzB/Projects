...
class UnitGaussianNormalizer(object):
    def __init__(self, x, eps=0.00001):
        ...
        self._device = next(iter(x)).device
        self.mean = torch.mean(x, 0).to(self._device )
        self.std = torch.std(x, 0).to(self._device )
        self.eps = torch.tensor(eps,device = self._device)
